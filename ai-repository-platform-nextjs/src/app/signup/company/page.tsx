...
export default function CompanySignup() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CompanySignupInputs>({ resolver: yupResolver(schema) });

  const onSubmit = async (data: CompanySignupInputs) => {
    setLoading(true);
    setFirebaseError('');
    try {
      await signUp(data.email, data.password, 'professional', data.companyName);
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-400 via-purple-300 to-pink-300 px-2 overflow-hidden">
      ...
      <div className="bg-white/90 p-6 sm:p-8 rounded-2xl shadow-2xl border border-blue-100 w-full max-w-xs sm:max-w-md flex flex-col items-center relative z-10">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2 drop-shadow text-center">Create Your Company Account</h2>
        <p className="text-gray-600 mb-6 text-center text-sm sm:text-base">Sign up to post problem statements and connect with top talent.</p>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 text-black bg-white/80 placeholder-gray-400"
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-green-600 to-blue-600 text-white py-2 rounded-lg font-semibold hover:from-green-700 hover:to-blue-700 transition text-base sm:text-lg shadow"
            disabled={loading}
          >
